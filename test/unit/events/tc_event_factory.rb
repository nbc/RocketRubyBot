...
class TestEventFactory < MiniTest::Test
  ...
  def test_coherence_events
    ...
    hash.each_pair do |k, v|
      event = RocketRubyBot::Events::EventFactory.builder to_openstruct(v)
      assert_equal k.to_sym, event.type
    end
  end

  def test_result_id_for_subscription
    event = RocketRubyBot::Events::EventFactory.builder(to_openstruct({ msg: 'ready', subs: ['4'] }))
    assert_equal '4', event.id
  end

  def test_result_id_for_other_commands
    event = RocketRubyBot::Events::EventFactory.builder(to_openstruct({ msg: 'result', id: '2', result: 'GENERAL' }))
    assert_equal '2', event.id
  end

  def test_log_unknown_event
    # FIXME
    RocketRubyBot::Events::EventFactory.builder(to_openstruct({ msg: 'unknown' }))
  end
end
