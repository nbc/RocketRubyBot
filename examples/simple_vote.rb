...
class SimpleVoteBot < RocketRubyBot::Bot
  setup do |client|
    ...
    subscriptions.update.each do |canal|
      client.stream_room_messages(room_id: canal.rid)
    end

    # we subscribe to stream to known when we are added to group
    client.stream_notify_user(user_id: config.user_id, sub: 'subscriptions-changed')
  end
